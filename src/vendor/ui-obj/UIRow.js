...
class UIRow {
  ...
  loadData(data) {
    ...
    if (self.getUIObj()) {
      const ds = self.getUIObj()
      ds.onCollectChanged(Enums.CollectionChangedAction.REPLACE, this)
  ...
  acceptChanges() {
    ...
    let dataSrcOriginRow = null
    if (this.getUIObj()) {
      this.getUIObj()._originRows.forEach(function(r) {
        if (r[ID_PROPERTY] === self[ID_PROPERTY]) {
          dataSrcOriginRow = r
        }
      })
    }
  ...
  rejectChanges() {
    // 丢弃修改
    // 放弃所有的修改，只对修改行有用
    if (this.dataRowState === Enums.DataRowState.MODIFIED) {
      this.dataRowState = Enums.DataRowState.UNCHANGED
      for (let i = 0; i < this.getUIObj().columns.length; i++) {
        const c = this.getUIObj().columns[i]
