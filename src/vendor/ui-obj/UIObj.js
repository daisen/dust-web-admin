...
import UIRelation from './UIRow'
import { serialize, format, ID_PROPERTY, ROW_SELECT_COLNAME, toStrLines, getLength, getScaleLength, getIntegerLength, eventuality } from './util'
...
class UIObj {
  ...
  init(dict) {
    ...
    this.objId = `${this.funcCode}_${this.uiObjCode}`
    Dict.addUiObj(this)
    return this
  }
  ...
  getPrintData() {
    ...
    for (let j = 0; j < self.getDetailRelations().length; j++) {
      ...
      printData.push(serialize(r.detail, false))
    }
    ...
    this.onRowDeleting(dr, needConfirm, function() {
      ...
      for (let j = 0; j < self._childRelations.length; j++) {
        ...
        if (dr.hasOwnProperty(r.detail.name)) {
          for (let i = 0; i < dr[r.detail.name].rows.length; i++) {
            dr[r.detail.name].rows[i].dataRowState = Enums.DataRowState.DELETED
          }
        }
      }
  ...
  acceptChanges() {
    ...
    for (let i = 0, len = this.rows.length; i < len; i++) {
      ...
      dr.acceptChanges()
      this._originRows.push(dr.clone)
    }
  ...
  getChanges(iteratee) {
    ...
    if (pr) {
      const masterRows = pr.master.getAllRows()
      for (let j = 0; j < masterRows.length; j++) {
        const mrow = masterRows[j]
        if (mrow.hasOwnProperty(pr.detail.name)) {
          for (let l = 0; l < mrow[pr.detail.name].rows.length; l++) {
            const drow = mrow[pr.detail.name].rows[l]
          ...
          if (mrow[pr.detail.name].deleted.length > 0) {
            for (let l = 0; l < mrow[pr.detail.name].deleted.length; l++) {
              const dDelRow = mrow[pr.detail.name].deleted[l]
  ...
  rejectChanges(deep) {
    ...
    if (this._originRows) {
      for (let i = 0; i < this._originRows.length; i++) {
        const originRow = this._originRows[i]
        const row = _.clone(originRow)
  ...
  addRelation(detail, masterColunms, detailColumns) {
    ...
    const relation = new UIRelation(this, detail, masterColunms, detailColumns)
  ...
  loadData(rows) {
    ...
    try {
      for (let i = 0; i < rows.length; i++) {
        ...
        const row = new UIRow(this, true)
        for (let j = 0; j < this.columns.length; j++) {
          const col = this.columns[j]
          if (rowData.hasOwnProperty(col.fieldName.toLowerCase())) {
            row[col.fieldName] = rowData[col.fieldName.toLowerCase()]
          }
    ...
    if (msgs.length === 0 && deep) {
      for (let i = 0; i < this._childRelations.length; i++) {
        ...
        if (this.getRequireDetail() && rel.detail.isEmpty()) {
          this._error = format(MSG.DS_REQUIREDETAIL, rel.detail.uiObjName)
          return false
        }
      }
    }
  ...
  getSchemaData(args) {
    ...
    // 存储过程不需要数据
    if (self.dataObjType === Enums.DataObjType.STOREDPROC) {
      args.nodata = true
    }
  ...
  save(callback, deep) {
    ...
    UIObj.requestSave(postData, function(isOk, data) {
      if (isOk) {
        const schemaData = data[self.uiObjCode]
  ...
  getQueryParams(args) {
    ...
    if (ds._parentRelation) {
      const masterRow = ds._parentRelation.master.currentRow
  ...
  getColumnParams() {
    const params = {}
    const ds = this
    if (ds.dataObjType === Enums.DataObjType.STOREDPROC) {
      for (let i = 0; i < ds.columns.length; i++) {
        const col = ds.columns[i]
  ...
  search(callback, args) {
    ...
    UIObj.requestSearch(postData, function(isOk, result) {
      if (isOk) {
        // 优先清除数据，保证数据的实时性
        self.clear()
        const schemaData = result[self.uiObjCode]
        self.setSchemaData(schemaData)
        self.acceptChanges()
      }
      if (callback) {
      ...
      UIObj.requestSearch(postData, function(isOk, data) {
        for (let j = 0; j < self._childRelations.length; j++) {
          ...
          if (isOk && data.hasOwnProperty(d.detail.uiObjCode)) {
            const result = data[d.detail.uiObjCode]
            d.detail.clear()
            d.detail.setSchemaData(result)
            d.detail.acceptChanges()
          }
        }
  ...
  getDetailsParams(dr, cb) {
    ...
    for (let i = 0; i < this._childRelations.length; i++) {
      ...
      if (dr && dr.dataRowState !== Enums.DataRowState.ADDED) {
        _.extend(searchData, r.detail.getSchemaData({
          masterRow: dr
        }))
      }
    }
  ...
  validateColumnWidth(dr, dc, value) {
    ...
    if (dc.dataType === Enums.DataType.STRING || dc.dataType === Enums.DataType.TEXT) {
      ...
      if (len > dc.dataWidth) {
        dr.setColumnError(dc, dc.dataSrc.onGetDataWidthErr(dr, dc, format(MSG.VALIDATE_LENGTH, dc.dispName, dc.dataWidth)))
        return false
      }
    }
    ...
    if (dc.dataType === Enums.DataType.NUMBER) {
      ...
      const realScale = dc.dataSrc.onGetPrecision(dr, dc)
      realInteger = realInteger > 0 ? realInteger : 0
      if (scale > realScale || integer > realInteger) {
        dr.setColumnError(dc, dc.dataSrc.onGetDataWidthErr(dr, dc, format(MSG.VALIDATE_LENGTH_NUM, dc.dispName, realInteger, realScale)))
        return false
      }
    }
 ...
 */
  validateColumnType(dr, dc, value) {
    ...
    if (!bRet) {
      dr.setColumnError(dc, dc.dataSrc.onGetDataTypeErr(dr, dc, format(MSG.VALIDATE_TYPE, dc.dispName, value)))
    }
    return bRet
  }
  ...
  valiateColumnRule(dr, dc, value) {
    ...
    if (!reg.test(value)) {
      dr.setColumnError(dc, dc.dataSrc.onGetDataRuleErr(dr, dc, format(MSG.VAILIDATE_RULE, dc.dispName, value)))
    }
  }
      ...
      // 严格校验,只有表参照和枚举生效
        if (dc.isStrictRef && !dc.getRefItem()) {
          dr.setColumnError(dc,
            dc.dataSrc.onGetDataStrictErr(dr, dc, format(MSG.VALIDTE_STRICTREF, dc.dispName, value)))
          return false
        }
      }
    }
      ...
      for (let j = 0; j < ds.rows.length; j++) {
        ...
        if (row === dr) {
          ...
          if (tag === curTag) {
            ...
            ds._error = MSG.LINE + index + ': ' + ds.onGetMultiMsg(dr)
            return false
          }
        }
      }
  ...
  validateDataRow(dr, onerror) {
    ...
    for (let k = 0; k < self.columns.length; k++) {
      ...
      if (col.isRequired && (!dr[col.fieldName] && dr[col.fieldName] !== 0)) {
        dr.setColumnError(col.fieldName, col.dispName + MSG.VALUE_REQUIRE, true)
      }
    }
    // 检查空行
    if (dr.isEmpty()) {
